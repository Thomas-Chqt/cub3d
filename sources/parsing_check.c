...
/*   Updated: 2023/10/05 16:35:25 by tchoquet         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	dfs(t_cubf *cubf, t_pos pos, t_bool **visited);
...
static int	dfs(t_cubf *cubf, t_pos pos, t_bool **visited)
{
	...
	if (pos.y >= cubf->size.h || pos.x >= cubf->size.w)
		return (set_error(NOT_CLOSED_ERROR), -1);
	...
	if (dfs(cubf, (t_pos){pos.x + 1, pos.y}, visited) != 0)
		return (-1);
	if (dfs(cubf, (t_pos){pos.x, pos.y + 1}, visited) != 0)
		return (-1);
	if (dfs(cubf, (t_pos){pos.x - 1, pos.y}, visited) != 0)
		return (-1);
	if (dfs(cubf, (t_pos){pos.x, pos.y - 1}, visited) != 0)
