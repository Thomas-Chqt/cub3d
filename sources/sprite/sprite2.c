...
/*   Updated: 2023/10/19 11:36:53 by tchoquet         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	add_new_barrel_sprite(t_list **lst, t_vec2f pos)
{
	...
	node->data = ft_calloc(1, sizeof(t_sprite));
	if (node->data == NULL)
		return (free(node), set_error(MALLOC_ERROR), -1);
	sp = (t_sprite *)node->data;
	sp->type = barrel;
	sp->pos = pos;
	sp->hp = 100;
	if (anim_from_path(&sp->idle_anim,
			"resources/sprites/barrel/idle/1.xpm", 1, 0) != 0)
		return (ft_lstclear(&node, (t_vf)free_sprite), -1);
